...
python3 scripts/omp-gateway.py restart   # 重新登录 Z Code / 切换套餐后同步
...
| `upstream.provider_id` | `builtin:bigmodel-coding-plan` | 取 Z Code 配置里哪个 provider 的密钥 |
| `upstream.credential_config_path` | `~/.zcode/v2/config.json` | Z Code 配置路径 |
...
| 上游 | 网关 | 说明 |
|---|---|---|
| 401 / 403 | 401 / 403 | 密钥失效：在 Z Code 重新登录后 `restart` |
...
- `zcode.z.ai/api/v1/zcode-plan/anthropic`（Z Code 自有套餐）未接入：当前账号未开通该套餐，且该端点可能要求 `Authorization: Bearer` 而非 `x-api-key`。
